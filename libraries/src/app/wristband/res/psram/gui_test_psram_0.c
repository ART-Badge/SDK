...
{
    [0]     = {
        .x = 0,   .y = 0,    .width = 390, .hight = 390,     .addr = 0x1000000,   .widget_id_type = ICON_BACKGROUND, \
...
/* change Here for UI */
UI_MenuTypeDef TestPSRAM0Menu =
{
    /* change Here for UI */
    .name = (const char *)"TestPSRAM0Menu",
    .MenuParent = NULL,
    .MenuSub = NULL,
...
static void TimerCallbackFun(RtkGuiTimerInfo_t *timer_info)
{
    ...
    if (memcmp(timer_info->menu_name,  TestPSRAM0Menu.name, strlen(TestPSRAM0Menu.name)))
    {
        return;
    }
    ...
    else if ((timer_id == 2) && (dynamic_timer != NULL))
    {
        ...
        {
            rtl_gui_prepare_frame_buffer(&TestPSRAM0Menu, NULL, &FrameBufferOrigin, false);
...
static void ConstructorCallbackFun(uint32_t type, void *pValue)
{
    ...
    rtl_gui_prepare_frame_buffer(&TestPSRAM0Menu, NULL, &FrameBufferOrigin, true);
    rtl_gui_update_detal(0, 0);
...
static void DisplayInfoCallbackFun(UI_MenuTypeDef *return_menu, void *argv)
{
    ...
    return_menu->current_max_icon = TestPSRAM0Menu.current_max_icon;
    /* do memcpy here */
    memcpy(return_menu->pWidgetList, TestPSRAM0Menu.pWidgetList,
           TestPSRAM0Menu.current_max_icon * sizeof(UI_WidgetTypeDef));
