...
static UI_MenuTypeDef *ui_local_cur = &Test454MenuMain;
static UI_MenuTypeDef *ui_local_right = &Test454MenuRight1;
static UI_MenuTypeDef *ui_local_left = &Test454MenuLeft1;
static UI_MenuTypeDef *ui_local_down = &Test454MenuDown1;
static UI_MenuTypeDef *ui_local_up = &Test454MenuUp1;


const static UI_WidgetTypeDef  icon[] =
{
    [0]     = {
        .x = 0,   .y = 0,    .width = LCD_WIDTH, .hight = LCD_HIGHT,     .addr = IMAGE_DEMO_454_MAIN_BIN,   .widget_id_type = ICON_BACKGROUND,
    },
    /*for test*/
    [1]     = {
        .x = 100,   .y = 100,    .width = 240, .hight = 32,     .addr = NULL,   .widget_id_type = ICON_STRING, \
        .touch_capability = false,
    },
};

/* change Here for UI */
UI_MenuTypeDef Test454MenuMain =
{
    /* change Here for UI */
    .name = (const char *)"Test454MenuMain",
    .MenuParent = NULL,
    .MenuSub = NULL,
...
static void TimerCallbackFun(RtkGuiTimerInfo_t *timer_info)
{
    ...
    if (memcmp(timer_info->menu_name,  ui_local_cur->name, strlen(ui_local_cur->name)))
    {
        return;
    }
    ...
    else if ((timer_id == 2) && (dynamic_timer != NULL))
    {
        ...
        {
            rtl_gui_prepare_frame_buffer(ui_local_cur, NULL, &FrameBufferOrigin, true);
...
static void DisplayInfoCallbackFun(UI_MenuTypeDef *return_menu, void *argv)
{
    ...
    return_menu->pWidgetList[1].u.string_mem.font_string_addr = (uint32_t)vol;
...
static void TouchCallbackFun(RtkTouchInfo_t *info)
{
    ...
    if (info->type == TOUCH_HOLD)
    {
        if (info->deltaX >= 0)
        {
            rtl_gui_prepare_frame_buffer(ui_local_left, NULL, &FrameBufferLeft, false);
        }
        else
        {
            rtl_gui_prepare_frame_buffer(ui_local_right, NULL, &FrameBufferRight, false);
        }
    ...
    else if (info->type == TOUCH_HOLD_Y)
    {
        if (info->deltaY >= 0)
        {
            rtl_gui_prepare_frame_buffer(ui_local_up, NULL, &FrameBufferUp, false);
        }
        else
        {
            rtl_gui_prepare_frame_buffer(ui_local_down, NULL, &FrameBufferDown, false);
        }
