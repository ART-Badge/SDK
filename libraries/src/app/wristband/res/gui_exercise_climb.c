...
static void StartIconTouchCb(uint32_t button_touch_type)
{
    if (button_touch_type == SHORT_TOUCH_BUTTON)
    {
        ...
        rtl_gui_menu_update(&ExerciseClimbMenu, NULL, NULL, NULL, NULL, NULL, 0, 0);
    }
}
...
#define CLIMB_PAGE_NUM      20

const static UI_WidgetTypeDef  exercisemode_climb_icons[CLIMB_PAGE_NUM] =
{
    [0]     = {
        .x = 0,   .y = 0,    .width = 240, .hight = 240,     .addr = EXERCISE_CLIMB_STOP_BACKGROUND_BIN,   .widget_id_type = ICON_BACKGROUND,
    },
    /*time*/
    [1]     = {//for time hour
...
UI_MenuTypeDef ExerciseClimbMenu =
{
    .name = (const char *)"ExerciseClimbMenu",
    .MenuParent = NULL,
    .MenuSub = NULL,
    ...
    .pWidgetList = (UI_WidgetTypeDef *)exercisemode_climb_icons,
    .current_max_icon = CLIMB_PAGE_NUM,
};
...
static void TimerCallbackFun(RtkGuiTimerInfo_t *timer_info)
{
    ...
    if (memcmp(timer_info->menu_name,  ExerciseClimbMenu.name, strlen(ExerciseClimbMenu.name)))
    {
        return;
    }
...
static void DisplayInfoCallbackFun(UI_MenuTypeDef *return_menu, void *argv)
{
    ...
    return_menu->current_max_icon = ExerciseClimbMenu.current_max_icon;
    /* do memcpy here */
    memcpy(return_menu->pWidgetList, ExerciseClimbMenu.pWidgetList,
           ExerciseClimbMenu.current_max_icon * sizeof(UI_WidgetTypeDef));
    //to do, set steps, distance, calorie, progress.
    if (is_started == false)
    {
        return_menu->pWidgetList[0].addr = EXERCISE_CLIMB_STOP_BACKGROUND_BIN;
        return_menu->pWidgetList[13].addr = COMMON_START_BIN;
    }
    else
    {
        return_menu->pWidgetList[0].addr = EXERCISE_CLIMB_START_BACKGROUND_BIN;
