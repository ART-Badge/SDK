...
{
    [0]     = {
        .x = 0,   .y = 0,    .width = 240, .hight = 240,     .addr = COMMON_CLOCK_WATCH_BIN,   .widget_id_type = ICON_BACKGROUND, \
        .touch_capability = true,   .widget_touch_func = CurrentMenuButtonTest
    },
};

/* change Here for UI */
UI_MenuTypeDef ClockChangeWatchMenu =
{
    /* change Here for UI */
    .name = (const char *)"ClockChangeWatchMenu",
    .MenuParent = NULL,
    .MenuSub = NULL,
...
static void TimerCallbackFun(RtkGuiTimerInfo_t *timer_info)
{
    ...
    if (memcmp(timer_info->menu_name,  ClockChangeWatchMenu.name, strlen(ClockChangeWatchMenu.name)))
    {
        return;
    }
...
static void ConstructorCallbackFun(uint32_t type, void *pValue)
{
    ...
    rtl_gui_menu_update(&ClockChangeWatchMenu, NULL, NULL, NULL, NULL, NULL, 0, 0);
...
static void DisplayInfoCallbackFun(UI_MenuTypeDef *return_menu, void *argv)
{
    ...
    return_menu->current_max_icon = ClockChangeWatchMenu.current_max_icon;
    /* do memcpy here */
    memcpy(return_menu->pWidgetList, ClockChangeWatchMenu.pWidgetList,
           ClockChangeWatchMenu.current_max_icon * sizeof(UI_WidgetTypeDef));
}
static void ButtonCallbackFun(uint8_t type)
{
...
static void TouchCallbackFun(RtkTouchInfo_t *info)
{
    ...
    {
        rtl_gui_menu_update(&ClockChangeWatchMenu, NULL, &ClockChangeSpecialMenu, NULL,
                            &ClockChangeFitbitMenu, NULL, info->deltaX, 0);
    }
    ...
    if (info->type == TOUCH_ORIGIN_FROM_X)
    {
        ...
        rtl_gui_menu_slide(&ClockChangeWatchMenu, NULL, &ClockChangeSpecialMenu, NULL,
                           &ClockChangeFitbitMenu, NULL, info->deltaX, 0, 0, 0);
    }
    else if (info->type == TOUCH_RIGHT_SLIDE)
    {
        os_timer_restart(&page_switch_timer, PAGE_SWITCH_TIMER_INTERVAL);
        rtl_gui_menu_slide(&ClockChangeWatchMenu, NULL, &ClockChangeSpecialMenu, NULL,
                           &ClockChangeFitbitMenu, NULL, info->deltaX, 240, 0, 0);
    }
    else if (info->type == TOUCH_LEFT_SLIDE)
    {
        os_timer_restart(&page_switch_timer, PAGE_SWITCH_TIMER_INTERVAL);
        rtl_gui_menu_slide(&ClockChangeWatchMenu, NULL, &ClockChangeSpecialMenu, NULL,
                           &ClockChangeFitbitMenu, NULL, info->deltaX, -240, 0,
                           0);
    }
    else if (info->type == TOUCH_INVALIDE)
    {
        rtl_gui_menu_update(&ClockChangeWatchMenu, NULL, &ClockChangeSpecialMenu, NULL,
                            &ClockChangeFitbitMenu, NULL, 0, 0);
    }
}
