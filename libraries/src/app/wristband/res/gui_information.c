...
static void InformationMenuButton(uint32_t button_touch_type);
...
static void InformationMenuButton(uint32_t button_touch_type)
{
    ...
    {
        APP_PRINT_INFO0("[InformationMenuButton]-->short button test!!");
        /* change Here for UI */
    ...
    {
        APP_PRINT_INFO0("[InformationMenuButton]-->long button test!!");
        /* change Here for UI */
        pCurrentMenu->MenuSub = &SettingMenu;
        pCurrentMenu->destructor_func();
        pCurrentMenu = pCurrentMenu->MenuSub;
...
{
    [0]     = {
        .x = 0,   .y = 0,    .width = 240, .hight = 240,     .addr = INFORMATION_BG_BIN,   .widget_id_type = ICON_BACKGROUND, \
        .touch_capability = true,  .widget_touch_func = InformationMenuButton
    },
};

/* change Here for UI */
UI_MenuTypeDef InformationMenu =
{
    /* change Here for UI */
    .name = (const char *)"InformationMenu",
    .MenuParent = NULL,
    .MenuSub = NULL,
...
static void TimerCallbackFun(RtkGuiTimerInfo_t *timer_info)
{
    ...
    if (memcmp(timer_info->menu_name,  InformationMenu.name, strlen(InformationMenu.name)))
    {
        return;
    }
...
static void ConstructorCallbackFun(uint32_t type, void *pValue)
{
    ...
    rtl_gui_menu_update(&InformationMenu, NULL, NULL, NULL, NULL, NULL, 0, 0);
...
static void DisplayInfoCallbackFun(UI_MenuTypeDef *return_menu, void *argv)
{
    ...
    return_menu->current_max_icon = InformationMenu.current_max_icon;
    /* do memcpy here */
    memcpy(return_menu->pWidgetList, InformationMenu.pWidgetList,
           InformationMenu.current_max_icon * sizeof(UI_WidgetTypeDef));
}
static void ButtonCallbackFun(uint8_t type)
{
