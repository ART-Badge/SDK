    ...
    spiffs_page_ix *objix_pix)
{
    ...
    res = _spiffs_rd(fs, SPIFFS_OP_T_OBJ_LU2 | SPIFFS_OP_C_READ, 0, addr, sizeof(spiffs_page_ix),
                     (u8_t *)pix);
...
static s32_t spiffs_rewrite_page(spiffs *fs, spiffs_page_ix cur_pix, spiffs_page_header *p_hdr,
...
static s32_t spiffs_rewrite_index(spiffs *fs, spiffs_obj_id obj_id, spiffs_span_ix data_spix,
                                  spiffs_page_ix new_data_pix, spiffs_page_ix objix_pix)
{
    s32_t res;
    spiffs_block_ix bix;
    int entry;
    ...
    if (objix_spix == 0)
    {
        ((spiffs_page_ix *)((u8_t *)fs->lu_work + sizeof(spiffs_page_object_ix_header)))[data_spix] =
            new_data_pix;
    }
    else
    {
        ((spiffs_page_ix *)((u8_t *)fs->lu_work + sizeof(spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(fs,
...
static s32_t spiffs_lookup_check_validate(spiffs *fs, spiffs_obj_id lu_obj_id,
                                          ...
                                          spiffs_page_ix cur_pix, spiffs_block_ix cur_block, int cur_entry, int *reload_lu)
{
        ...
        ((lu_obj_id == SPIFFS_OBJ_ID_FREE) && (p_hdr->flags & SPIFFS_PH_FLAG_USED) == 0))
    {
        ...
        *reload_lu = 1;
        ...
        if (p_hdr->flags & SPIFFS_PH_FLAG_INDEX)
        {
            ...
            res = spiffs_object_get_data_page_index_reference(fs, p_hdr->obj_id, p_hdr->span_ix, &ref_pix,
                                                              &objix_pix);
            if (res == SPIFFS_ERR_NOT_FOUND)
            {
        ...
        else
        {
            ...
            res = spiffs_obj_lu_find_id_and_span(fs, p_hdr->obj_id | SPIFFS_OBJ_ID_IX_FLAG, p_hdr->span_ix,
                                                 cur_pix, 0);
            if (res == SPIFFS_ERR_NOT_FOUND)
            {
    ...
    if (lu_obj_id != SPIFFS_OBJ_ID_FREE && lu_obj_id != SPIFFS_OBJ_ID_DELETED)
    {
        ...
        if ((p_hdr->obj_id | SPIFFS_OBJ_ID_IX_FLAG) != (lu_obj_id | SPIFFS_OBJ_ID_IX_FLAG))
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    res = spiffs_obj_lu_find_id_and_span(fs, lu_obj_id | SPIFFS_OBJ_ID_IX_FLAG, p_hdr->span_ix, 0,
                                                         &objix_pix_lu);
                    if (res == SPIFFS_ERR_NOT_FOUND)
                    {
                        res = SPIFFS_OK;
                    ...
                    res = spiffs_obj_lu_find_id_and_span(fs, p_hdr->obj_id | SPIFFS_OBJ_ID_IX_FLAG, p_hdr->span_ix, 0,
                                                         &objix_pix_ph);
                    if (res == SPIFFS_ERR_NOT_FOUND)
                    {
                        res = SPIFFS_OK;
                    ...
                    if (objix_pix_ph == 0 || objix_pix_lu == 0)
                    {
                        ...
                        res = spiffs_obj_lu_find_id_and_span(fs, p_hdr->obj_id & ~SPIFFS_OBJ_ID_IX_FLAG, 0, 0,
                                                             &data_pix_ph);
                        if (res == SPIFFS_ERR_NOT_FOUND)
                        {
                            res = SPIFFS_OK;
                 ...
                 ((lu_obj_id & SPIFFS_OBJ_ID_IX_FLAG) == 0 && (p_hdr->flags & SPIFFS_PH_FLAG_INDEX) == 0))
        {
            ...
            res = spiffs_obj_lu_find_id_and_span(fs, lu_obj_id & ~SPIFFS_OBJ_ID_IX_FLAG, p_hdr->span_ix,
                                                 cur_pix, &data_pix);
            if (res == SPIFFS_ERR_NOT_FOUND)
            {
                res = SPIFFS_OK;
            ...
            res = spiffs_obj_lu_find_id_and_span(fs, lu_obj_id | SPIFFS_OBJ_ID_IX_FLAG, p_hdr->span_ix, cur_pix,
                                                 &objix_pix_d);
            if (res == SPIFFS_ERR_NOT_FOUND)
            {
                res = SPIFFS_OK;
        ...
        else if ((p_hdr->flags & SPIFFS_PH_FLAG_FINAL))
        {
            ...
            *reload_lu = 1;
            res = spiffs_object_get_data_page_index_reference(fs, lu_obj_id, p_hdr->span_ix, &ref_pix,
                                                              &objix_pix);
            if (res == SPIFFS_ERR_NOT_FOUND)
            {
                // no object with this id, so remove page safely
...
static s32_t spiffs_lookup_check_v(spiffs *fs, spiffs_obj_id obj_id, spiffs_block_ix cur_block,
                                   int cur_entry,
                        ...
                        if (res == SPIFFS_OK)
                        {
                            ...
                            else
                            {
                                ...
                                if (((p_hdr.obj_id & ~SPIFFS_OBJ_ID_IX_FLAG) == rp_hdr.obj_id) &&
                                    ((rp_hdr.flags & (SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_DELET | SPIFFS_PH_FLAG_USED |
...
static s32_t spiffs_object_index_consistency_check_v(spiffs *fs, spiffs_obj_id obj_id,
                                                     ...
                                                     int cur_entry, const void *user_const_p, void *user_var_p)
{
        ...
        (obj_id & SPIFFS_OBJ_ID_IX_FLAG))
    {
        ...
        if (p_hdr.span_ix == 0 &&
            (p_hdr.flags & (SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_FINAL | SPIFFS_PH_FLAG_DELET |
        ...
        if ((p_hdr.flags & (SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_FINAL | SPIFFS_PH_FLAG_DELET |
                            SPIFFS_PH_FLAG_IXDELE)) ==
...
s32_t spiffs_object_index_consistency_check(spiffs *fs)
{
    ...
    res = spiffs_obj_lu_find_entry_visitor(fs, 0, 0, 0, 0, spiffs_object_index_consistency_check_v, 0,
                                           &obj_id_log_ix,
                                           0, 0);
    if (res == SPIFFS_VIS_END)
    {
