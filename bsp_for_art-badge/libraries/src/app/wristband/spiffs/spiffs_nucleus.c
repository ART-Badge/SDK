...
static s32_t spiffs_page_data_check(spiffs *fs, spiffs_fd *fd, spiffs_page_ix pix,
                                    spiffs_span_ix spix)
{
    s32_t res = SPIFFS_OK;
...
#if !SPIFFS_READ_ONLY
static s32_t spiffs_page_index_check(spiffs *fs, spiffs_fd *fd, spiffs_page_ix pix,
                                     spiffs_span_ix spix)
{
    s32_t res = SPIFFS_OK;
    if (pix == (spiffs_page_ix) - 1)
    ...
    void *user_var_p)
{
    ...
    if (ph.obj_id == obj_id &&
        ...
        (ph.flags & (SPIFFS_PH_FLAG_FINAL | SPIFFS_PH_FLAG_DELET | SPIFFS_PH_FLAG_USED)) ==
    ...
    while (map_spix < map_spix_end)
    {
        ...
        if (objix_spix == 0)
        {
            ...
            objix_data_pix = ((spiffs_page_ix *)((u8_t *)objix + sizeof(
                                                     spiffs_page_object_ix_header)))[map_spix];
        }
        else
        {
            // get data page from object index page
            objix_data_pix = ((spiffs_page_ix *)((u8_t *)objix + sizeof(
                                                     spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(fs, map_spix)];
        }
    ...
    spiffs_page_ix *pix)
{
    ...
    // find free entry
    res = spiffs_obj_lu_find_free(fs, fs->free_cursor_block_ix, fs->free_cursor_obj_lu_entry, &bix,
                                  &entry);
    SPIFFS_CHECK_RES(res);
    ...
    {
        res = _spiffs_wr(fs,  SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_UPDT,
                         0, SPIFFS_OBJ_LOOKUP_ENTRY_TO_PADDR(fs, bix, entry) + sizeof(spiffs_page_header) + page_offs, len,
                         data);
        SPIFFS_CHECK_RES(res);
    }
    ...
    spiffs_page_ix *dst_pix)
{
    ...
    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_UPDT,
                     0, SPIFFS_BLOCK_TO_PADDR(fs, SPIFFS_BLOCK_FOR_PAGE(fs,
                                                                        free_pix)) + SPIFFS_OBJ_LOOKUP_ENTRY_FOR_PAGE(fs, free_pix) * sizeof(spiffs_page_ix),
    ...
    spiffs_page_ix pix)
{
    ...
    res = _spiffs_wr(fs, SPIFFS_OP_T_OBJ_LU | SPIFFS_OP_C_DELE,
                     0,
                     SPIFFS_BLOCK_TO_PADDR(fs, SPIFFS_BLOCK_FOR_PAGE(fs, pix)) + SPIFFS_OBJ_LOOKUP_ENTRY_FOR_PAGE(fs,
                             pix) * sizeof(spiffs_page_ix),
                     sizeof(spiffs_obj_id),
                     (u8_t *)&d_obj_id);
    SPIFFS_CHECK_RES(res);
    ...
    spiffs_page_ix *new_pix)
{
    ...
    res = spiffs_page_move(fs, fd == 0 ? 0 : fd->file_nbr, (u8_t *)objix_hdr, obj_id, 0, objix_hdr_pix,
                           &new_objix_hdr_pix);
        ...
        if (spix == 0)
        {
            if (ev != SPIFFS_EV_IX_DEL)
            {
                SPIFFS_DBG("       callback: setting fd "_SPIPRIfd":"_SPIPRIid"(fdoffs:"_SPIPRIi" offs:"_SPIPRIi") objix_hdr_pix to "_SPIPRIpg", size:"_SPIPRIi"\n",
                           SPIFFS_FH_OFFS(fs, cur_fd->file_nbr), cur_fd->obj_id, cur_fd->fdoffset, cur_fd->offset, new_pix,
                           new_size);
...
s32_t spiffs_object_append(spiffs_fd *fd, u32_t offset, u8_t *data, u32_t len)
{
    ...
    res = spiffs_gc_check(fs, len + SPIFFS_DATA_PAGE_SIZE(
                              fs)); // add an extra page of data worth for meta
    if (res != SPIFFS_OK)
    {
    ...
    while (res == SPIFFS_OK && written < len)
    {
        ...
        else
        {
            ...
            {
                // get data page from object index header page
                data_page = ((spiffs_page_ix *)((u8_t *)objix_hdr + sizeof(
                                                    spiffs_page_object_ix_header)))[data_spix];
            }
            else
            {
                // get data page from object index page
                data_page = ((spiffs_page_ix *)((u8_t *)objix + sizeof(spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(
                        fs, data_spix)];
            }
        ...
        if (cur_objix_spix == 0)
        {
            ...
            ((spiffs_page_ix *)((u8_t *)objix_hdr + sizeof(spiffs_page_object_ix_header)))[data_spix] =
                data_page;
        ...
        else
        {
            ...
            ((spiffs_page_ix *)((u8_t *)objix + sizeof(spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(fs,
                                                                                data_spix)] = data_page;
...
s32_t spiffs_object_modify(spiffs_fd *fd, u32_t offset, u8_t *data, u32_t len)
{
    ...
    while (res == SPIFFS_OK && written < len)
    {
        ...
        if (cur_objix_spix != prev_objix_spix)
        {
            ...
            if (written > 0)
            {
                ...
                else
                {
                    ...
                    res = spiffs_page_move(fs, fd->file_nbr, (u8_t *)objix, fd->obj_id, 0, cur_objix_pix,
                                           &new_objix_pix);
        ...
        {
            // get data page from object index header page
            orig_data_pix = ((spiffs_page_ix *)((u8_t *)objix_hdr + sizeof(
                                                    spiffs_page_object_ix_header)))[data_spix];
        }
        else
        {
            // get data page from object index page
            orig_data_pix = ((spiffs_page_ix *)((u8_t *)objix + sizeof(
                                                    spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(fs, data_spix)];
        }
    ...
    if (cur_objix_spix != 0)
    {
        ...
        res2 = spiffs_page_move(fs, fd->file_nbr, (u8_t *)objix, fd->obj_id, 0, cur_objix_pix,
                                &new_objix_pix);
    ...
    u8_t remove_full)
{
    ...
    {
        u8_t flags = ~(SPIFFS_PH_FLAG_USED | SPIFFS_PH_FLAG_INDEX | SPIFFS_PH_FLAG_FINAL |
    ...
    while (cur_size > new_size)
    {
        ...
        {
            // get data page from object index header page
            data_pix = ((spiffs_page_ix *)((u8_t *)objix_hdr + sizeof(
                                               spiffs_page_object_ix_header)))[data_spix];
            ((spiffs_page_ix *)((u8_t *)objix_hdr + sizeof(spiffs_page_object_ix_header)))[data_spix] =
                SPIFFS_OBJ_ID_FREE;
        }
        ...
        {
            // get data page from object index page
            data_pix = ((spiffs_page_ix *)((u8_t *)objix + sizeof(spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(
                    fs, data_spix)];
            ((spiffs_page_ix *)((u8_t *)objix + sizeof(spiffs_page_object_ix)))[SPIFFS_OBJ_IX_ENTRY(fs,
    ...
    else
    {
        ...
        res = spiffs_page_move(fs, fd->file_nbr, (u8_t *)objix_hdr, fd->obj_id, 0, objix_pix,
                               &new_objix_pix);
        SPIFFS_CHECK_RES(res);
        spiffs_cb_object_event(fs, (spiffs_page_object_ix *)objix_hdr,
                               SPIFFS_EV_IX_UPD, fd->obj_id, objix->p_hdr.span_ix, new_objix_pix, 0);
    ...
    u8_t *dst)
{
    ...
    while (cur_offset < offset + len)
    {
        ...
        len_to_read = MIN(len_to_read,
                          SPIFFS_DATA_PAGE_SIZE(fs) - (cur_offset % SPIFFS_DATA_PAGE_SIZE(fs)));
        ...
        SPIFFS_DBG("read: offset:"_SPIPRIi" rd:"_SPIPRIi" data spix:"_SPIPRIsp" is data_pix:"_SPIPRIpg" addr:"_SPIPRIad"\n",
                   ...
                   (u32_t)(SPIFFS_PAGE_TO_PADDR(fs,
                                                data_pix) + sizeof(spiffs_page_header) + (cur_offset % SPIFFS_DATA_PAGE_SIZE(fs))));
        if (len_to_read <= 0)
        {
        ...
        res = _spiffs_rd(
                  fs, SPIFFS_OP_T_OBJ_DA | SPIFFS_OP_C_READ,
                  fd->file_nbr,
                  SPIFFS_PAGE_TO_PADDR(fs, data_pix) + sizeof(spiffs_page_header) + (cur_offset %
...
static s32_t spiffs_obj_lu_find_free_obj_id_bitmap_v(spiffs *fs, spiffs_obj_id id,
...
static s32_t spiffs_obj_lu_find_free_obj_id_compact_v(spiffs *fs, spiffs_obj_id id,
                                                      spiffs_block_ix bix, int ix_entry,
                                                      const void *user_const_p, void *user_var_p)
{
    (void)user_var_p;
                                     ...
                                     const u8_t *conflicting_name)
{
    ...
    while (res == SPIFFS_OK && free_obj_id == SPIFFS_OBJ_ID_FREE)
    {
        ...
        else
        {
            ...
            state.compaction = (state.max_obj_id - state.min_obj_id) / ((SPIFFS_CFG_LOG_PAGE_SZ(fs) / sizeof(
                                                                             u8_t)));
            SPIFFS_DBG("free_obj_id: COMP min:"_SPIPRIid" max:"_SPIPRIid" compact:"_SPIPRIi"\n",
                       state.min_obj_id, state.max_obj_id, state.compaction);

            memset(fs->work, 0, SPIFFS_CFG_LOG_PAGE_SZ(fs));
            res = spiffs_obj_lu_find_entry_visitor(fs, 0, 0, 0, 0, spiffs_obj_lu_find_free_obj_id_compact_v,
                                                   &state, 0, 0, 0);
            if (res == SPIFFS_VIS_END) { res = SPIFFS_OK; }
            SPIFFS_CHECK_RES(res);
...
static u32_t spiffs_hash(spiffs *fs, const u8_t *name)
{
    ...
    u8_t c;
    int i = 0;
    while (name[i++] && i < SPIFFS_OBJ_NAME_LEN)   // howie modify for compiler waring
    {
        c = name[i];
        hash = (hash * 33) ^ c;
    }
