...
TEST(write_cache)
{
    ...
    TEST_CHECK(0 == memcmp(&buf[0], &fbuf[SPIFFS_CFG_LOG_PAGE_SZ(FS) / 2],
                           SPIFFS_CFG_LOG_PAGE_SZ(FS) * 2));
    res = SPIFFS_close(FS, fd);
    TEST_CHECK(res >= 0);
...
TEST(write_big_file_chunks_index)
{
    int size = ((50 * SPIFFS_CFG_PHYS_SZ(FS)) / 100);
    printf("  filesize %i\n", size);
    int res = test_create_and_write_file("bigfile", size,
                                         SPIFFS_DATA_PAGE_SIZE(FS) * SPIFFS_OBJ_HDR_IX_LEN(FS));
    TEST_CHECK(res >= 0);
    res = read_and_verify("bigfile");
    TEST_CHECK(res >= 0);
...
{
    TEST_CHECK(create_and_read_back(SPIFFS_DATA_PAGE_SIZE(FS) * (SPIFFS_PAGES_PER_BLOCK(
                                                                     FS) - SPIFFS_OBJ_LOOKUP_PAGES(FS)) * 2,
...
{
    TEST_CHECK(create_and_read_back(SPIFFS_DATA_PAGE_SIZE(FS) * (SPIFFS_PAGES_PER_BLOCK(
                                                                     FS) - SPIFFS_OBJ_LOOKUP_PAGES(FS)) * 4,
...
TEST(bad_index_1)
{
    ...
    u32_t addr = SPIFFS_PAGE_TO_PADDR(FS,
                                      pix) + sizeof(spiffs_page_object_ix_header) + 2 * sizeof(spiffs_page_ix);
