...
TEST(page_cons1)
{
    ...
    u32_t addr = SPIFFS_PAGE_TO_PADDR(FS,
                                      pix) + sizeof(spiffs_page_object_ix_header) + 0 * sizeof(spiffs_page_ix);
...
TEST(page_cons2)
{
    ...
    u32_t addr = SPIFFS_PAGE_TO_PADDR(FS,
                                      pix) + sizeof(spiffs_page_object_ix_header) + 1 * sizeof(spiffs_page_ix);
...
TEST(index_cons4)
{
    ...
    u32_t addr = SPIFFS_PAGE_TO_PADDR(FS, pix) + offsetof(spiffs_page_header, flags);
    u8_t flags = 0xff & ~(SPIFFS_PH_FLAG_FINAL | SPIFFS_PH_FLAG_USED | SPIFFS_PH_FLAG_INDEX |
