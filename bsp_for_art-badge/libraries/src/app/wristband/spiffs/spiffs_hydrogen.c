...
#if SPIFFS_CACHE
    fs->cache = cache;
    fs->cache_size = (cache_size > (SPIFFS_CFG_LOG_PAGE_SZ(fs) * 32)) ? SPIFFS_CFG_LOG_PAGE_SZ(
                         fs) * 32 : cache_size;
...
spiffs_file SPIFFS_open_by_dirent(spiffs *fs, struct spiffs_dirent *e, spiffs_flags flags,
                                  spiffs_mode mode)
{
...
spiffs_file SPIFFS_open_by_page(spiffs *fs, spiffs_page_ix page_ix, spiffs_flags flags,
                                spiffs_mode mode)
{
...
static s32_t spiffs_hydro_read(spiffs *fs, spiffs_file fh, void *buf, s32_t len)
{
    ...
    spiffs_file tmp_local_file = SPIFFS_FH_UNOFFS(fs, fh);
    fh = tmp_local_file;
        ...
        if (len < (s32_t)SPIFFS_CFG_LOG_PAGE_SZ(fs))
        {
            ...
            if (fd->cache_page)
            {
...
#ifdef _SPIFFS_TEST
                {
                    ...
                    intptr_t __b = sizeof(spiffs_cache) + cache->cpage_count * (sizeof(spiffs_cache_page) +
...
{
    SPIFFS_API_DBG("%s "_SPIPRIfd " "_SPIPRIi " %s\n", __func__, fh, offs, (const char *[]) {"SET", "CUR", "END", "???"}[MIN(
                whence, 3)]);
    SPIFFS_API_CHECK_CFG(fs);
    SPIFFS_API_CHECK_MOUNT(fs);
    SPIFFS_LOCK(fs);
