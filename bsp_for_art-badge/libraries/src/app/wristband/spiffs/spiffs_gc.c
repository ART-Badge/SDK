    ...
    spiffs *fs, u16_t max_free_pages)
{
    ...
    while (res == SPIFFS_OK && blocks--)
    {
        ...
        if (res == SPIFFS_OK &&
            deleted_pages_in_block + free_pages_in_block == SPIFFS_PAGES_PER_BLOCK(
                fs) - SPIFFS_OBJ_LOOKUP_PAGES(fs) &&
...
//  }
    ...
    do
    {
        SPIFFS_GC_DBG("\ngc_check #"_SPIPRIi": run gc free_blocks:"_SPIPRIi" pfree:"_SPIPRIi" pallo:"_SPIPRIi" pdele:"_SPIPRIi" ["_SPIPRIi"] len:"_SPIPRIi" of "_SPIPRIi"\n",
                      tries,
                      fs->free_blocks, free_pages, fs->stats_p_allocated, fs->stats_p_deleted,
    ...
    char fs_crammed)
{
    ...
    int max_candidates = MIN(fs->block_count,
    ...
    while (res == SPIFFS_OK && gc.state != FINISHED)
    {
                    ...
                    if (obj_id == gc.cur_obj_id)
                    {
                        ...
                        else
                        {
                            ...
                            if (gc.cur_objix_spix == 0)
                            {
                                ...
                                ((spiffs_page_ix *)((u8_t *)objix_hdr + sizeof(spiffs_page_object_ix_header)))[p_hdr.span_ix] =
                                    new_data_pix;
        ...
        case FIND_OBJ_DATA:
            if (gc.obj_id_found)
            {
                ...
                res = spiffs_obj_lu_find_id_and_span(fs, gc.cur_obj_id | SPIFFS_OBJ_ID_IX_FLAG, gc.cur_objix_spix,
                                                     0, &objix_pix);
        ...
        case MOVE_OBJ_DATA:
            {
                ...
                if (gc.cur_objix_spix == 0)
                {
                    ...
                    res = spiffs_object_update_index_hdr(fs, 0, gc.cur_obj_id | SPIFFS_OBJ_ID_IX_FLAG, gc.cur_objix_pix,
                                                         fs->work, 0, 0, 0, &new_objix_pix);
                ...
                else
                {
                    ...
                    res = spiffs_page_move(fs, 0, fs->work, gc.cur_obj_id | SPIFFS_OBJ_ID_IX_FLAG, 0, gc.cur_objix_pix,
                                           &new_objix_pix);
