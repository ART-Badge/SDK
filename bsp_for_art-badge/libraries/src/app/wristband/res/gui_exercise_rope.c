...
static void StartIconTouchCb(uint32_t button_touch_type)
{
    if (button_touch_type == SHORT_TOUCH_BUTTON)
    {
        ...
        rtl_gui_menu_update(&ExerciseRopeMenu, NULL, NULL, NULL, NULL, NULL, 0, 0);
    }
}
...
#define ROPE_PAGE_NUM      20

const static UI_WidgetTypeDef  exercise_rope_icon[ROPE_PAGE_NUM] =
{
    [0]     = {
        .x = 0,   .y = 0,    .width = 240, .hight = 240,     .addr = EXERCISE_ROPE_STOP_BACKGROUND_BIN,   .widget_id_type = ICON_BACKGROUND,
    },
    /*time*/
    [1]     = {//for time hour
...
/* change Here for UI */
UI_MenuTypeDef ExerciseRopeMenu =
{
    /* change Here for UI */
    .name = (const char *)"ExerciseRopeMenu",
    .MenuParent = NULL,
    .MenuSub = NULL,
    ...
    .pWidgetList = (UI_WidgetTypeDef *)exercise_rope_icon,
    .current_max_icon = ROPE_PAGE_NUM,
};
...
static void TimerCallbackFun(RtkGuiTimerInfo_t *timer_info)
{
    ...
    if (memcmp(timer_info->menu_name,  ExerciseRopeMenu.name, strlen(ExerciseRopeMenu.name)))
    {
        return;
    }
...
static void DisplayInfoCallbackFun(UI_MenuTypeDef *return_menu, void *argv)
{
    ...
    return_menu->current_max_icon = ExerciseRopeMenu.current_max_icon;
    /* do memcpy here */
    memcpy(return_menu->pWidgetList, ExerciseRopeMenu.pWidgetList,
           ExerciseRopeMenu.current_max_icon * sizeof(UI_WidgetTypeDef));

    if (is_started == false)
    {
        return_menu->pWidgetList[0].addr = EXERCISE_ROPE_STOP_BACKGROUND_BIN;
        return_menu->pWidgetList[13].addr = COMMON_START_BIN;
    }
    else
    {
        return_menu->pWidgetList[0].addr = EXERCISE_ROPE_START_BACKGROUND_BIN;
