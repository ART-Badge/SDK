...
{
    [0]     = {
        .x = 0,   .y = 0,    .width = 240, .hight = 240,     .addr = HEART_RATE_BG_BIN,   .widget_id_type = ICON_BACKGROUND, \
        .touch_capability = false, .widget_touch_func = CurrentMenuButtonTest
    },
    [1]     = {
        .x = 147,   .y = 19,    .width = 19, .hight = 36,     .addr = COMMON_P19X36RED_BIN,   .widget_id_type = ICON_NUM, \
        .touch_capability = false
    },
    [2]     = {
        .x = 184,   .y = 67,    .width = 49, .hight = 44,     .addr = HEART_RATE_BRIGHT_BIN,   .widget_id_type = ICON_WIDGET, \
        .touch_capability = false
    },
    [3]     = {
        .x = 197,   .y = 118,    .width = 27, .hight = 19,     .addr = COMMON_BMP_BIN,   .widget_id_type = ICON_WIDGET, \
        .touch_capability = false
    },

    [4]     = {.x = 87,   .y = 226,   .width = 8,     .hight = 8,    .addr = COMMON_POINT_GRAY_BIN,  .widget_id_type = ICON_WIDGET},
    [5]     = {.x = 99,   .y = 226,   .width = 8,     .hight = 8,    .addr = COMMON_POINT_GRAY_BIN,  .widget_id_type = ICON_WIDGET},
    [6]     = {.x = 110,  .y = 226,   .width = 8,     .hight = 8,    .addr = COMMON_POINT_GRAY_BIN,  .widget_id_type = ICON_WIDGET},
    [7]     = {.x = 122,  .y = 226,   .width = 8,     .hight = 8,    .addr = COMMON_POINT_WHITE_BIN,  .widget_id_type = ICON_WIDGET},
    [8]     = {.x = 133,  .y = 226,   .width = 8,     .hight = 8,    .addr = COMMON_POINT_GRAY_BIN,  .widget_id_type = ICON_WIDGET},
    [9]     = {.x = 145,  .y = 226,   .width = 8,     .hight = 8,    .addr = COMMON_POINT_GRAY_BIN,  .widget_id_type = ICON_WIDGET},
};

/* change Here for UI */
UI_MenuTypeDef HeartRateMenu =
{
    /* change Here for UI */
    .name = (const char *)"HeartRateMenu",
    .MenuParent = NULL,
    .MenuSub = NULL,
...
static void TimerCallbackFun(RtkGuiTimerInfo_t *timer_info)
{
    ...
    if (memcmp(timer_info->menu_name,  HeartRateMenu.name, strlen(HeartRateMenu.name)))
    {
        return;
    }
...
static void ConstructorCallbackFun(uint32_t type, void *pValue)
{
    ...
    rtl_gui_menu_update(&HeartRateMenu, NULL, NULL, NULL, NULL, NULL, 0, 0);
...
static void DisplayInfoCallbackFun(UI_MenuTypeDef *return_menu, void *argv)
{
    ...
    return_menu->current_max_icon = HeartRateMenu.current_max_icon;
    /* do memcpy here */
    memcpy(return_menu->pWidgetList, HeartRateMenu.pWidgetList,
           HeartRateMenu.current_max_icon * sizeof(UI_WidgetTypeDef));
}
static void ButtonCallbackFun(uint8_t type)
{
...
static void TouchCallbackFun(RtkTouchInfo_t *info)
{
    ...
    {
        rtl_gui_menu_update(&HeartRateMenu, NULL, &BPMenu, NULL, &StepMenu, NULL, info->deltaX, 0);
    }
    ...
    if (info->type == TOUCH_ORIGIN_FROM_X)
    {
        ...
        rtl_gui_menu_slide(&HeartRateMenu, NULL, &BPMenu, NULL, &StepMenu, NULL, info->deltaX, 0, 0, 0);
    }
    else if (info->type == TOUCH_RIGHT_SLIDE)
    {
        os_timer_restart(&page_switch_timer, PAGE_SWITCH_TIMER_INTERVAL);
        rtl_gui_menu_slide(&HeartRateMenu, NULL, &BPMenu, NULL, &StepMenu, NULL, info->deltaX, 240, 0, 0);
    }
    ...
    {
        rtl_gui_menu_update(&HeartRateMenu, NULL, &BPMenu, NULL, &StepMenu, NULL, 0, 0);
    }
}
