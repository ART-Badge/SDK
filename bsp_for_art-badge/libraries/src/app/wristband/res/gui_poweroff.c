...
static void PoweroffYesButton(uint32_t button_touch_type);
static void PoweroffNoButton(uint32_t button_touch_type);

#define POWEROFF_PAGE_NUM      3

const static UI_WidgetTypeDef  poweroff_icons[POWEROFF_PAGE_NUM] =
{
    [0]     = {
        .x = 0,     .y = 0,     .width = 240,   .hight = 240,    .addr = SETTING_POWEROFF_BG_BIN,   .widget_id_type = ICON_BACKGROUND, \
        .touch_capability = false,
    },
    [1]     = {
        .x = 36,     .y = 120,    .width = 66,    .hight = 66,     .addr = NULL,   .widget_id_type = ICON_WIDGET, \
        .touch_capability = true,     .widget_touch_func = PoweroffYesButton
    },
    [2]     = {
        .x = 136,    .y = 120,    .width = 66,    .hight = 66,     .addr = NULL,   .widget_id_type = ICON_WIDGET, \
        .touch_capability = true,     .widget_touch_func = PoweroffNoButton
    },
};

UI_MenuTypeDef PoweroffMenu =
{
    .name = (const char *)"PoweroffMenu",
    .MenuParent = NULL,
    .MenuSub = NULL,
    ...
    .pWidgetList = (UI_WidgetTypeDef *)poweroff_icons,
    .current_max_icon = POWEROFF_PAGE_NUM,
};
...
static void TimerCallbackFun(RtkGuiTimerInfo_t *timer_info)
{
    ...
    if (memcmp(timer_info->menu_name,  PoweroffMenu.name, strlen(PoweroffMenu.name)))
    {
        return;
    }
...
static void ConstructorCallbackFun(uint32_t type, void *pValue)
{
    ...
    rtl_gui_menu_update(&PoweroffMenu, NULL, NULL, NULL, NULL, NULL, 0, 0);
...
static void DisplayInfoCallbackFun(UI_MenuTypeDef *return_menu, void *argv)
{
    ...
    return_menu->current_max_icon = PoweroffMenu.current_max_icon;
    /* do memcpy here */
    memcpy(return_menu->pWidgetList, PoweroffMenu.pWidgetList,
           PoweroffMenu.current_max_icon * sizeof(UI_WidgetTypeDef));
