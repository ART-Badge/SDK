...
typedef enum
{
    AIRSYNC_ONLINE_NOTIFICATION_EVENT = 0,
    AIRSYNC_UPLOAD_PEDO_PRIVATE_EVENT,
    AIRSYNC_UPLOAD_PEDO_WECHAT_EVENT,
    AIRSYNC_HEARTTICK_EVENT,
    AIRSYNC_AUTH_EVENT,
    AIRSYNC_INIT_EVENT,
} AirsyncSendType;


void airsync_send_stream(uint8_t *pBuffer, uint32_t length);
void set_data_complete_callback(uint8_t length, uint8_t *current_pointer);
...
void resolve_airsync_response_cmd(SendDataResponse *sendDataResp);
void resolve_airsync_push_cmd(RecvDataPush *recvDatPush);
