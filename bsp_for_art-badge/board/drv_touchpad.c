...
#ifndef PIN_KEY_UP
    #define PIN_KEY_UP      P2_5
#endif

#ifndef PIN_KEY_DOWN
    #define PIN_KEY_DOWN    P2_6
#endif

#ifndef PIN_KEY_LEFT
    #define PIN_KEY_LEFT    P2_7
#endif

#ifndef PIN_KEY_RIGHT
    #define PIN_KEY_RIGHT   H_0
...
static uint8_t btn_state = USER_BUTTON_UNKNOWN;
...
static void common_btn_evt_cb(void *arg)
{
    ...
    /* simulate touching left */
    if (flex_button_event_read(&user_button[USER_BUTTON_LEFT]) == FLEX_BTN_PRESS_LONG_HOLD)
    {
        left_long_press_touch();
    }

    /* simulate touching right */
    if (flex_button_event_read(&user_button[USER_BUTTON_RIGHT]) == FLEX_BTN_PRESS_LONG_HOLD)
    {
        right_long_press_touch();
    }
}
...
static void user_button_init(void)
{
    ...
    for (i = 0; i < USER_BUTTON_MAX; i ++)
    {
        ...
        user_button[i].short_press_start_tick = FLEX_MS_TO_SCAN_CNT(200);
        user_button[i].long_press_start_tick = FLEX_MS_TO_SCAN_CNT(2000);
        user_button[i].long_hold_start_tick = FLEX_MS_TO_SCAN_CNT(40000);
...
int flex_button_main(void)
{
    ...
    tid = rt_thread_create("flex_btn", button_scan, RT_NULL, 1024, 20, 20);
