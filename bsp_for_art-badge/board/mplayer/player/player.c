...
MSH_CMD_EXPORT(player_stop, player_stop);
...
int player_show(player_t player)
{
    ...
    rt_kprintf("Playback progress: %02d%%  Volume level: %02d%%\n", percent, player->volume);
...
int player_start(player_t player)
{
    ...
    /* 创建动态线程 */
    player->play_thread = rt_thread_create("mplayer",
                                           player_entry, player,
                                           512, 20, 20);
...
// MSH_CMD_EXPORT(player_start,player_start);