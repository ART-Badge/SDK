...
/* OAT progress process callback */
typedef void (*rt_ota_progress_cb)(uint32_t percentage);
...
/**
 * OTA registration progress processing callback function
 *
 * @param callback Progress processing callback function
 *
 * @return < 0: parameter callback is null
 *         = 0: register success
 */
int rt_ota_progress_cb_register(rt_ota_progress_cb callback);
