...
#define EMQ_MSG_TYPE_BASE       (0x00)
#define EMQ_MSG_TYPE_BUFF       (0x01)
#define EMQ_MSG_TYPE_UNKNOWN    (0xFF)

#define EMQ_MSG_SUBTYPE_UNKNOWN (0xFF)
...
struct emq_container
{
    ...
    rt_uint8_t msgtype;                     /* emq message type. BASE or BUFF */
    rt_uint8_t subtype;                     /* emq message subtype. application usage */
    rt_uint16_t prvtoken;                   /* private tokens */
};
...
struct emq_ep
{
    ...
    rt_slist_t ep_node;             /* append queue list */
};
...
struct emq_queue
{
    ...
    rt_slist_t  free;                   /* free message list */
    rt_slist_t  head;                   /* message list head */
    rt_slist_t  *tail;                  /* message list tail */

    rt_slist_t  ep_list;                /* point all ep */
    void (*recv_fn)(emq_t *queue);      /* recv msg callback */
    rt_list_t notify_node;              /* notify node */
};
...
const char* emq_queue_name(emq_t* queue, const char *name);
...
struct emq_ep *emq_mapping_find(emq_t *queue, const struct emq_hub *hub,
    rt_uint16_t channel, rt_base_t mapping);
/* endpoint */
void emq_ep_remove(struct emq_ep *ep);

/* recv */
rt_err_t emq_recv(emq_t *queue, rt_base_t *msg, rt_base_t *mapping, rt_int32_t timeout);
rt_base_t emq_recv_forever(emq_t* queue, rt_base_t* mapping);
...
/* message container */
rt_err_t emq_container_recv(emq_t *queue, struct emq_container *container, rt_int32_t timeout);
int emq_container_send(const struct emq_container *container, const struct emq_hub *hub, rt_uint16_t channel, int urgent);

#ifdef EMQ_USING_HOOK
/* emq queue hook */
void emq_attach_sethook(void (*hook)(emq_t* queue));
void emq_detach_sethook(void (*hook)(emq_t* queue));
void emq_trytake_sethook(void (*hook)(emq_t* queue));
void emq_take_sethook(void (*hook)(emq_t* queue));
void emq_put_sethook(void (*hook)(const struct emq_hub* hub, rt_uint16_t channel));
#endif
