...
static int open_db_file(fdb_db_t db, uint32_t addr, bool clean)
{
    ...
    if (sec_addr != db->cur_sec || fd <= 0 || clean) {
...
#ifdef PKG_USING_FASTFS 
        struct dfs_filesystem *fs = NULL;
        fs = dfs_filesystem_lookup(path);
        if (fs && strcmp(fs->ops->name, "fastfs") == 0) {
            db->is_fastfs = true;
#ifdef FDB_USING_FAL_MODE
            if ((db->fastfs_part = fal_partition_find(fs->dev_id->parent.name)) == NULL) {
                db->cur_file = -1;
                return db->cur_file;
            }
        ...
        if (clean) {
            ...
            fd = open(path, open_flags, 0777);
            if (fd <= 0) {
                FDB_INFO("Error: open (%s) file failed.\n", path);
            }
            else {
                if (db->is_fastfs) {
                    ftruncate(fd, db->sec_size);
                }
                close(fd);
                fd = -1;
            }
        ...
        if (db->is_fastfs && (fd > 0)) {
            ioctl(fd, RT_FIOGETOFFSETADDR, &db->fastfs_file_offset);
        }
#else
        if (fd > 0) {
            close(fd);
...
#if defined(FDB_USING_FAL_MODE) && defined(PKG_USING_FASTFS)
        if (db->is_fastfs) {
            if (fal_partition_read(db->fastfs_part, db->fastfs_file_offset + addr, (uint8_t *)buf, size) < 0) {
                result = FDB_READ_ERR;
            }
            return result;
        }
#endif /* FDB_USING_FAL_MODE && PKG_USING_FASTFS */
...
#if defined(FDB_USING_FAL_MODE) && defined(PKG_USING_FASTFS)
        if (db->is_fastfs) {
            if (fal_partition_write(db->fastfs_part, db->fastfs_file_offset + addr, (uint8_t *)buf, size) < 0) {
...
#if defined(FDB_USING_FAL_MODE) && defined(PKG_USING_FASTFS)
        if (db->is_fastfs) {
            if (fal_partition_erase(db->fastfs_part, db->fastfs_file_offset, size) < 0) {
