...
#define HEARTBEAT_PERIOD_MS (1000/20)
...
uint8_t adv_data[] = {
    ...
    // Name
    0x10, BLUETOOTH_DATA_TYPE_COMPLETE_LOCAL_NAME, 'P', 'e', 'r', 's', 'i', 'm', 'W', 'e', 'a', 'r', '_', ' ', ' ', ' ', ' ',
...
static void le_counter_setup(void)
{
    ...
    uint8_t mac_id[5] = {0};
    rt_snprintf((char*)mac_id,5,"%02X%02X",*(rt_uint8_t *)(UID_BASE + 2),*(rt_uint8_t *)(UID_BASE + 0));
    rt_memcpy(adv_data + 16,mac_id,strlen((char*)mac_id));// offset 16 for mac id
...
#define GATT_SEND_MAX_SIZE (ATT_DEFAULT_MTU-3)
...
int bt_stack_blufi_send(uint8_t *string, uint32_t length)
{
    ...
    {
        LOG_D("===start send string====");
        send_buffer_complete = 0;
        send_buffer_index = 0;
    }
    else
    {
        LOG_E("===le_notification_enabled:%d ===", le_notification_enabled);
        return -1;
    }
    return 0;
}
...
void bt_send_api(void)
{
    ...
    LOG_D("======counter_string_len:%d======", temp_length);
...
void ble_send_data_test(const char *buf,rt_uint16_t length)
{
    uint8_t wifi_status = 1;
    uint32_t temp_length = 0;
    LOG_D("send_string as  %.*s,length %d", length,buf,length);
    bt_stack_blufi_send((uint8_t *)buf, length);
}
...
typedef void (*ble_data_trans_cb)(uint8_t *buffer,rt_uint16_t length);
extern ble_data_trans_cb g_ble_recv_data_cb, g_ble_send_data_cb;
static void packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    UNUSED(channel);
    UNUSED(size);
    ...
    case HCI_EVENT_PACKET:
        ...
        case ATT_EVENT_CAN_SEND_NOW:
            ...
            LOG_D("====send length= %d ==left:%s===", send_length, (uint8_t *)(counter_string + send_buffer_index));
            if (send_buffer_index >= counter_string_len)
            {
                //send complete
                g_ble_send_data_cb((void*)counter_string, send_length);
...
static int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    if (att_handle == ATT_CHARACTERISTIC_FF01_01_CLIENT_CONFIGURATION_HANDLE)
    {
        ...
        LOG_I("BLE notify status changed, enabled: %d", le_notification_enabled);
    }
    if (att_handle == ATT_CHARACTERISTIC_FF01_01_VALUE_HANDLE)
    {
        LOG_D("\r\n recv data from as : %.*s length :%d\r\n", buffer_size,buffer,buffer_size);
        g_ble_recv_data_cb((uint8_t*) buffer, (rt_uint16_t) buffer_size);
        #if 0  // use for save distribution network data, if we use distribution network
