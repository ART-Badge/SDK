...
typedef js_value_t (*js_external_handler_t)(const js_value_t function_obj,
                                            const js_value_t this_val, const js_value_t args_p[],
...
typedef bool (*js_objects_foreach_t)(const js_value_t object, void *user_data_p);

typedef bool (*js_object_property_foreach_t)(const js_value_t property_name,
                                             const js_value_t property_value, void *user_data_p);
...
bool js_value_is_cesu8_string(const js_char_t *value, const js_size_t size);
...
js_size_t js_string_to_char_buffer(const js_value_t value, js_char_t *buffer_p,
                                   js_size_t buffer_size);
js_size_t js_string_to_utf8_char_buffer(const js_value_t value, js_char_t *buffer_p,
                                        js_size_t buffer_size);
js_size_t js_substring_to_char_buffer(const js_value_t value, js_length_t start_pos,
                                      js_length_t end_pos, js_char_t *buffer_p,
                                      js_size_t buffer_size);
js_size_t js_substring_to_utf8_char_buffer(const js_value_t value, js_length_t start_pos,
                                           js_length_t end_pos, js_char_t *buffer_p,
                                           js_size_t buffer_size);
...
js_value_t js_create_error_sz(js_error_t error_type, const js_char_t *message_p,
                              js_size_t message_size);
...
js_value_t js_has_own_property(const js_value_t obj_val, const js_value_t prop_name_val);
...
bool js_get_object_native_pointer(const js_value_t obj_val, void **out_native_pointer_p,
                                  const js_object_native_info_t **out_pointer_info_p);
void js_set_object_native_pointer(const js_value_t obj_val, void *native_pointer_p,
                                  const js_object_native_info_t *native_info_p);

bool js_objects_foreach(js_objects_foreach_t foreach_p, void *user_data);
...
js_value_t js_get_backtrace(uint32_t max_depth);

js_value_t js_parse(const js_char_t *resource_name_p, size_t resource_name_length,
                    const js_char_t *source_p, size_t source_size, uint32_t parse_opts);
js_value_t js_run(const js_value_t func_val);
js_value_t js_eval(const js_char_t *source_p, size_t source_size, uint32_t parse_opts);

js_value_t js_load_function_snapshot(const uint32_t *function_snapshot_p,
                                     const size_t function_snapshot_size, size_t func_index,
                                     uint32_t exec_snapshot_opts);

int js_module_register(const char *name, js_value_t (*func)(void));
