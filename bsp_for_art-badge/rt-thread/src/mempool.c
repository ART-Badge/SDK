...
void *rt_mp_alloc(rt_mp_t mp, rt_int32_t time)
{
    ...
    /* calculate the maximum number of memory pools used */
    block_max_count = mp->block_total_count - mp->block_free_count;
    if (block_max_count > 0)
    {
        mp->block_max_count = block_max_count > mp->block_max_count ? block_max_count : mp->block_max_count;
    }
