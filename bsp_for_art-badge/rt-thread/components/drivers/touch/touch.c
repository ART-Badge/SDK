...
static rt_err_t rt_touch_control(rt_device_t dev, int cmd, void *args)
{
    ...
    case RT_TOUCH_CTRL_POWER_ON:
        result = touch->ops->touch_control(touch, RT_TOUCH_CTRL_POWER_ON, args);
        break;
    case RT_TOUCH_CTRL_POWER_OFF:
        result = touch->ops->touch_control(touch, RT_TOUCH_CTRL_POWER_OFF, args);
        break;
    case RT_TOUCH_CTRL_GET_STATUS:
        if (args)
        {
            result = touch->ops->touch_control(touch, RT_TOUCH_CTRL_GET_STATUS, args);
        }
        else
        {
            result = -RT_ERROR;
