...
static void _pm_change_sleep_mode(struct rt_pm *pm)
{
    ...
    else
    {
        ...
        {
            timeout_tick = rt_lptimer_next_timeout_tick();
            ...
            else
            {
                ...
                {
                    pm->sleep_mode = PM_TICKLESS_THRESHOLD_MODE;
                }
                else
                {
                    if (pm->ops->timer_start)
...
/**
 * This function will let current module work with PM_SLEEP_IDLE mode.
 *
 * @param module_id the pm module id
 *
 * @return NULL
...
/**
 * The specified module release the requested PM_SLEEP_IDLE mode
 *
 * @param module_id the pm module id
 *
 * @return none
