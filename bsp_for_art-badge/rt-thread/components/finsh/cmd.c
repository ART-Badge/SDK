...
long list_mempool(void)
{
    ...
    rt_kprintf("%-*.s block total free max  suspend thread totle size\n", maxlen, item_title); object_split(maxlen);
    rt_kprintf(     " ----  ----  ---- ---- -------------- ----------\n");
    do
    {
        next = list_get_next(next, &find_arg);
        {
            ...
            for (i = 0; i < find_arg.nr_out; i++)
            {
                ...
                {
                    rt_kprintf("%-*.*s %04d  %04d  %04d %04d %14d %10d\n",
                            maxlen, RT_NAME_MAX,
                            mp->parent.name,
                            mp->block_size,
                            ...
                            mp->block_max_count,
                            suspend_thread_count,
                            (mp->block_size * mp->block_total_count));
                ...
                totle_size += mp->block_size *  mp->block_total_count;
            }
        }
    }
    while (next != (rt_list_t*)RT_NULL);

    /* calculate the total memory size used by mempool */
    rt_kprintf("-------- ----  ----  ---- ---- -------------- ----------\n");
    rt_kprintf("                                              %10d\n", totle_size);
