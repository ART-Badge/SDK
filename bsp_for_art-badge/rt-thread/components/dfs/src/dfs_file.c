...
static void copyfile(const char *src, const char *dst)
{
    ...
    else
    {
        ...
        rt_kprintf("Read %s file stat failed\n", src);
        
        return;
    }

    fs = dfs_filesystem_lookup(dst);
    if (fs && strcmp(fs->ops->name, "fastfs") == 0) {
        result = dfs_file_ftruncate(&fd, file_size);
        if(result < 0)
        {
            rt_free(block_ptr);
            dfs_file_close(&src_fd);
            ...
            rt_kprintf("Set %s file length %d failed:%d\n", dst, file_size, result);
