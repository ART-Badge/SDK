...
void rt_memheap_free(void *ptr)
{
    ...
    {
        rt_kprintf("bad magic:0x%08x @ memheap\n", header_ptr->magic);
    }
...
void *rt_malloc(rt_size_t size)
{
    ...
    else
    {
        ...
        LOG_D("malloc => 0x%08x : %d", ptr, size);
    }
#endif
...
void *rt_realloc(void *rmem, rt_size_t newsize)
{
    ...
    else
    {
        ...
        LOG_D("realloc => 0x%08x : %d", new_ptr, newsize);
    }
#endif
...
void *rt_calloc(rt_size_t count, rt_size_t size)
{
    ...
    if (ptr == RT_NULL)
    {
        LOG_E("calloc[%d x %d] => NULL", count, size);
    }
    else
    {
        LOG_D("calloc => 0x%08x : %d", ptr, count * size);
    }
#endif
