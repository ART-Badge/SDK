...
void rt_pm_module_delay_sleep(rt_uint8_t module_id, rt_tick_t timeout)
{
    ...
    pm->module_status[module_id].timeout = rt_tick_get() + timeout;

    rt_hw_interrupt_enable(level);
}

/**
 * This function check if all modules idle status.
 */
static rt_bool_t _pm_device_check_idle(void)
{
    ...
    for (int i = 0; i < PM_MODULE_MAX_ID; i++)
    {
        ...
        {
            if (rt_tick_get() > pm->module_status[i].timeout)
            {
                       ...
                       void                   *user_data)
{
    ...
    #error "[PM ERR] IDLE stack size too small!"
#endif
...
static void rt_pm_dump_status(void)
{
    ...
    rt_kprintf("pm module request:\n");
    for (index = 0; index < PM_MODULE_MAX_ID; index ++)
    {
        if (pm->module_status[index].req_status != 0x00)
            rt_kprintf("| module[%d] req", index);
        if (pm->module_status[index].busy_flag == RT_TRUE)
            rt_kprintf(" module[%d] busy\n", index);
    }
    rt_kprintf("\n ------------------------------\n");
}
