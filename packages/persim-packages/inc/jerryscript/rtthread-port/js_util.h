...
#define DECLARE_HANDLER(NAME)                           \
    static js_value_t                                   \
    NAME ## _handler (const js_value_t func_value,      \
                        const js_value_t this_value,    \
                        const js_value_t args[],        \
                        const js_length_t args_cnt )

#define REGISTER_HANDLER(NAME)      \
    js_handler_register_global ( (js_char_t *)# NAME, NAME ## _handler)
...
#define REGISTER_METHOD(OBJ, NAME)  \
    js_handler_register (OBJ, # NAME, NAME ## _handler)
#define REGISTER_METHOD_ALIAS(OBJ, NAME, HANDLER) \
    js_handler_register (OBJ, # NAME, HANDLER)
...
void *js_get_pointer_property(const js_value_t obj, const char *name, const js_object_native_info_t **out_native_info_p);
...
void js_set_pointer_property(const js_value_t obj, const char *name, void *value, const js_object_native_info_t *native_info_p);
...
js_value_t js_call_function_x(const js_value_t obj, const char *name,
                               const js_value_t args[], js_size_t args_cnt);
...
int js_read_file(const char* filename, void** script);
#ifndef JS_READ_FILE
#define JS_READ_FILE(filename, script) js_read_file(filename, script)
#endif

int js_util_lock_init(void);
int js_util_lock_deinit(void);
