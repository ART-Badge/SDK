...
typedef struct ezxml_cache
{
    char *xml;
    ezxml_t root;
} ezxml_cache_t;

ezxml_cache_t *ezxml_structure_parse(const char* fn);
int ezxml_structure_release(const char *fn);
ezxml_cache_t *ezxml_structure_find(const char *fn);
int ezxml_structure_put(ezxml_cache_t *cache);
void ezxml_structure_empty(struct string_table *str_table);
